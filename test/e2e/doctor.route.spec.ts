...
describe("Doctor route ECE Test",()=>{
    ...
    describe("Register a doctor",()=>{
        ...
        it("Register doctor is successful",async()=>{
            ...
            const response=await appRequest.get("/doctor/"+"doctorinfo?username=" +Doctorinformation.username );
            expect(response.statusCode).toBeLessThan(300)
            expect(response.body).toHaveProperty("email",Doctorinformation.email)
            expect(response.body).toHaveProperty("username",Doctorinformation.username)
            expect(response.body).toHaveProperty("firstname",Doctorinformation.firstname)
            expect(response.body).toHaveProperty("lastname",Doctorinformation.lastname)
    ...
    describe("Register a patient",()=>{
        ...
        it("Register patient is successful",async()=>{
            const response=await appRequest.get("/patient/"+"patientinfo?username=" +patinetinformation.username );
            expect(response.status).toBeLessThan(300)
            expect(response.body).toHaveProperty("email",patinetinformation.email)
            expect(response.body).toHaveProperty("username",patinetinformation.username)
            expect(response.body).toHaveProperty("firstname",patinetinformation.firstname)
            expect(response.body).toHaveProperty("lastname",patinetinformation.lastname)
        ...
        describe('post a record', () => {
            beforeEach(async()=>{
             ...
             it ("post record successfully",async()=>{
                const response=await appRequest.post("/doctor/"+"record-add" ).send({
        ...
        describe("update doctor record",()=>{
            ...
            it("update doctor record successfully",async()=>{
                const response=await appRequest.post("/doctor/"+"doctorinfo" ).send({
                    username:Doctorinformation.username,
                    firstname:Doctorinformation.firstname,
                    lastname:Doctorinformation.lastname,
                    email:Doctorinformation.email,
        ...
        describe("update doctor record2",()=>{
            ...
            it("update doctor record not successfull",async()=>{
                const response=await appRequest.post("/doctor/"+"doctorinfo" ).send({
                    username:Doctorinformation.username,
                    firstname:Doctorinformation.firstname,
                    lastname:Doctorinformation.lastname,
        ...
        describe("get patient list",()=>{
            beforeEach(async()=>{
             ...
             it ("add patient successfully",async()=>{
                const response=await appRequest.post("/doctor/"+"patientlist" ).send({
                    patientUsername: patinetinformation.username,
                    doctorUsername: Doctorinformation.username,
                    action:"add"
                });
        ...
        describe("check if doctor has no  patient",()=>{
            it("check if doctor has no patient",async()=>{
                ...
                const response=await appRequest.get("/doctor/"+"patientlist?username=" +Doctorinformation.username );
                expect(response.statusCode).toBeGreaterThan(300)
            })
        })
        ...
        describe("check if doctor has patient",()=>{
            ...
            it("get list of patients", async () => {
                ...
                const response = await appRequest.get(
                    "/doctor/" + "getpatients?username=" + Doctorinformation.username + "&page=1"
                );
            ...
            //     // get all patient records
            //     const response = await appRequest.get(
            //         "/doctor/" + "getpatients?username=" + Doctorinformation.username + "&page=adfas"
            //     );
            //     expect(response.status).toBeGreaterThan(300)
            // })
